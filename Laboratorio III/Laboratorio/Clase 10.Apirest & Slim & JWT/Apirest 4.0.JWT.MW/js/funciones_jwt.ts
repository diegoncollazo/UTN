...
const APIREST_JWT:string = "http://localhost/prog_3/apirestV4-JWT-MW/BACKEND/JWT/api/";
...
{   
    ...
    .done(function (resultado:any) {
        ...
        let alerta:string = ArmarAlert('Usuario válido!');
...
{    
    ...
    .done(function (resultado:any) {
        ...
        let tabla:string = ArmarTablaCDs(resultado);
...
{   
    ...
    else
    {
        cds.forEach(cd => {
            ...
            tabla += "<tr><td>"+cd.id+"</td><td>"+cd.titulo+"</td><td>"+cd.cantante+"</td><td>"+cd.año+"</td><th>"+
            "<a href='#' class='btn' data-action='modificar' data-obj_cd='"+JSON.stringify(cd)+"' title='Modificar'"+
            " data-toggle='modal' data-target='#ventana_modal_cd' ><span class='fas fa-edit'></span></a>"+
            "<a href='#' class='btn' data-action='eliminar' data-obj_cd='"+JSON.stringify(cd)+"' title='Eliminar'"+
            " data-toggle='modal' data-target='#ventana_modal_cd' ><span class='fas fa-times'></span></a>"+
            "</td></tr>";
        });
    }
...
{  
    ...
    $.ajax({
        ...
        url: APIREST_JWT + "cd/",
        dataType: "json",
        data: {"cadenaJson":JSON.stringify(dato)},
        headers : {"token":jwt},
        async: true
    })
    .done(function (resultado:any) {

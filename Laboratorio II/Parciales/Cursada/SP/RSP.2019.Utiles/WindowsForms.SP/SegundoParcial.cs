...
namespace WindowsForms.SP
{
    public partial class SegundoParcial : Form
    {
        ...
        private void c_gomas_EventoPrecio(object sender, EventArgs e)
        {
            ...
            bool retorno = Ticketeadora.ImprimirTicket(this.c_gomas);

            if (retorno)
                MessageBox.Show("Impresión de ticket correcta.");
            else
        ...
        private void btnVerLog_Click(object sender, EventArgs e)
        {
            ...
            DialogResult retorno = this.openFileDialog1.ShowDialog();//Reemplazar por la llamada al método correspondiente del OpenFileDialog

            if (retorno == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void btnPunto5_Click(object sender, EventArgs e)
        {
            ...
            if (this.lapiz.Xml())
                MessageBox.Show("Lápiz serializado correctamente");
            else
                MessageBox.Show("Lápiz no serializado.");

            if (((IDeserializa)this.lapiz).Xml(out Lapiz auxLapiz))
            {
                MessageBox.Show("Lápiz serializado correctamente");
                MessageBox.Show(((Lapiz)auxLapiz).ToString());
            }
            else
            {
                MessageBox.Show("Lápiz no serializado.");
        ...
        // Sincronizar los cambios del DataTable con la DataBase
